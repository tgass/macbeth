...
spec = do
  describe "Board Config Encoding" $ do
    ...
    it "no tile config" $ encode (BoardConfig True Nothing Nothing Nothing Nothing :: BoardConfigFormat)
  ...
  describe "Board Config Decoding" $ do
    ...
    it "default board config" $ decode "{\"blackTile\":\"hexb49664\",\"showCapturedPieces\":false,\"whiteTile\":\"hexffffff\"}"
      `shouldBe` Just (BoardConfig False (Just $ TileRGB 255 255 255) (Just $ TileRGB 180 150 100) Nothing Nothing :: BoardConfigFormat)

    it "board config with tile filepath" $ decode "{\"blackTile\":\"wood_blk.bmp\",\"showCapturedPieces\":false,\"whiteTile\":\"hexffffff\"}"
      `shouldBe` Just (BoardConfig False (Just $ TileRGB 255 255 255) (Just $ TileFile "wood_blk.bmp") Nothing Nothing :: BoardConfigFormat)
    ...
    it "board config with no tile config" $ decode "{\"showCapturedPieces\":false}"
      `shouldBe` Just (BoardConfig False Nothing Nothing Nothing Nothing :: BoardConfigFormat)

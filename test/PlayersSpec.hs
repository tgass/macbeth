...
spec =
  describe "Players" $ do
    ...
    it "bughouse partner offer" $ parseOnly partnerOffer (BS.pack "GuestZTCG offers to be your bughouse partner; type \"partner GuestZTCG\" to accept.\n")
      `shouldBe` Right (PartnerOffer (UserHandle "GuestZTCG" []))

    it "bughouse partner accepted" $ parseOnly partnerAccepted (BS.pack "GuestCCGS agrees to be your partner.\n")
      `shouldBe` Right (PartnerAccepted (UserHandle "GuestCCGS" []))

    it "bughouse partner declined" $ parseOnly partnerDeclined (BS.pack "GuestCCGS declines the partnership request.\n")
      `shouldBe` Right (PartnerDeclined (UserHandle "GuestCCGS" []))

    it "bughouse partner not open" $ parseOnly partnerNotOpen (BS.pack "\NAK6\SYN84\SYNzerowin is not open for bughouse.\n\ETB\n")
